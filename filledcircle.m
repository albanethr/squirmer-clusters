...
% This function explores the dynamics of a circle of squirmers with a central squirmer, in the lubrication regime 
...
D = max( (epsis/2+1)./sin(pi/(N-1)), 2 + epsis);
X(:,1:N-1,1) = [D*cos((1:1:N-1)*2*pi./(N-1));h.*ones(1,(N-1));D*sin((1:1:(N-1))*2*pi./(N-1))];
E(:,1:N-1,1) = [-cos(theta0).*cos((1:1:(N-1))*2*pi./(N-1));sin(theta0).*ones(1,(N-1));-cos(theta0).*sin((1:1:(N-1))*2*pi./(N-1))];
X(:,N,1)= [0;h;0];
E(:,N,1)= [0; 1 ; 0]; % rand(1,3);%[0;-1;0];


% % center one
% D = (0.2/2+1)./sin(pi/(N-1));
% X(:,1:(N-1),1) = [D*cos((1:1:(N-1))*2*pi./(N-1));h.*ones(1,(N-1));D*sin((1:1:(N-1))*2*pi./(N-1))];
% X(:,1:N,1) = [X(:,1:(N-1),1), [0;h;0]] ;
% E(:,1:(N-1),1) = [-cos(theta0).*cos((1:1:(N-1))*2*pi./(N-1));sin(theta0).*ones(1,(N-1));-cos(theta0).*sin((1:1:(N-1))*2*pi./(N-1))]; E(:,:,1) = [E(:,1:(N-1),1), [0;1;0]] ;
